...
class Alp_Parceiros
{
  ...
  public static function setup(): void
  {
    self::$entity = (new Alp_Entity())
      ->create_post_type('Parceiro', 'Parceiros', 'parceiro', false, 'networking', true, true);

    self::create_metaboxes();
  }

  /**
   * Estabelece os metaboxes de Parceiro.
   * @return void
   */
  public static function create_metaboxes(): void
  {
    self::$entity
      ->create_metaboxes()
      ->add_metabox_box('', 'Parceiros')
      ->add_metabox_field_image('Logo', 'logo', 1, 12)
      ->render();
  }

  /**
   * Recebe um array com os valores dos campos do Parceiro.
  ...
  public function render_card_parceiro(int $id, $css_classes = ''): string
  {
    $logo = get_post_meta($id, 'parceiro_logo', true);
    ...
    return $this->html('frontend/views/avulsos/parceiros/card-parceiro', $args);
  }

  public function render_parceiros($swiper_class = '', $titulo = 'Parceiros', $subtitulo = 'Nossas parcerias estratégicas'): string
  {
    $query_args = [
      'post_type' => 'parceiro',
      'posts_per_page' => -1,
    ...
    if ($query->have_posts()) {
      while ($query->have_posts()) {
        ...
        $cards .= $this->render_card_parceiro(get_the_ID(), 'swiper-slide');
      }

      wp_reset_postdata();

      return $this->html('frontend/views/avulsos/parceiros/section-parceiros', [
        'cards' => $cards,
        'titulo' => $titulo,
        'subtitulo' => $subtitulo,
 ...
 * Hooks
 */
add_action('after_setup_theme', ['Alp_Parceiros', 'setup']);
